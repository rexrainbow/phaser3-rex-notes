...
const RetrieveImports = function(folderPath?: any) {
    ...
    try {
        fs.readdirSync(folderPath).forEach(function(file?: any) {
            ...
            GetImportStatement(lines).forEach(function(importStatement?: any) {
                GetImportNames(importStatement).forEach(function(item?: any) {
...
const GetImportStatement = function(lines?: any) {
...
const GetImportNames = function(importStatement?: any) {
    ...
    if (name.indexOf('{') === -1) {
        ...
        name.replace(/[\{\}]/g, '').split(',').forEach(function(name?: any) {
            name = name.trim();
            importNames.push({
                name: name,
                path: importPath,
