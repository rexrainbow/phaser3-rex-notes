import { Utils as PhaserUtils, Math as PhaserMath } from 'phaser';
import EaseMoveTo from './EaseMoveTo';
import EaseMoveFrom from './EaseMoveFrom';
import { WaitComplete } from '../../utils/promise/WaitEvent';

const IsPlainObject = PhaserUtils.Objects.IsPlainObject;
const DistanceBetween = PhaserMath.Distance.Between;

export default {
    onInitEaseMove() {
        var gameObject = this;
        var easeMove = this._easeMove;
        // Route 'complete' of easeMove to gameObject
        easeMove.completeEventName = undefined;
        easeMove.on('complete', function() {
            if (easeMove.completeEventName) {
                gameObject.emit(easeMove.completeEventName, gameObject);
                easeMove.completeEventName = undefined;
            }
        })
    },

    moveFrom(duration?: any, x?: any, y?: any, ease?: any, destroyMode?: any) {
        if (IsPlainObject(duration)) {
            var config = duration;
            x = config.x;
            y = config.y;
            if (config.hasOwnProperty('speed')) {
                duration = (DistanceBetween(x, y, this.x, this.y) * 1000) / config.speed;
            } else {
                duration = config.duration;
            }

            ease = config.ease;
        }

        var isInit = (this._easeMove === undefined);

        this._easeMove = EaseMoveFrom(this, duration, x, y, ease, destroyMode, this._easeMove);

        if (isInit?: any) {
            this.onInitEaseMove();
        }

        this._easeMove.completeEventName = 'movefrom.complete';

        return this;
    },

    moveFromPromise(duration?: any, x?: any, y?: any, ease?: any, destroyMode?: any) {
        this.moveFrom(duration, x, y, ease, destroyMode);
        return WaitComplete(this._easeMove);
    },

    moveFromDestroy(duration?: any, x?: any, y?: any, ease?: any) {
        this.moveFrom(duration, x, y, ease, true);
        return this;
    },

    moveFromDestroyPromise(duration?: any, x?: any, y?: any, ease?: any) {
        this.moveFromDestroy(duration, x, y, ease);
        return WaitComplete(this._easeMove);
    },

    isRunningMoveFrom() {
        return this._easeMove && (this._easeMove.completeEventName = 'movefrom.complete');
    },

    moveTo(duration?: any, x?: any, y?: any, ease?: any, destroyMode?: any) {
        if (IsPlainObject(duration)) {
            var config = duration;
            x = config.x;
            y = config.y;
            if (config.hasOwnProperty('speed')) {
                duration = (DistanceBetween(x, y, this.x, this.y) * 1000) / config.speed;
            } else {
                duration = config.duration;
            }

            ease = config.ease;
        }

        var isInit = (this._easeMove === undefined);

        this._easeMove = EaseMoveTo(this, duration, x, y, ease, destroyMode, this._easeMove);

        if (isInit?: any) {
            this.onInitEaseMove();
        }

        this._easeMove.completeEventName === 'moveto.complete';

        return this;
    },

    moveToPromise(duration?: any, x?: any, y?: any, ease?: any, destroyMode?: any) {
        this.moveTo(duration, x, y, ease, destroyMode);
        return WaitComplete(this._easeMove);
    },

    moveToDestroy(duration?: any, x?: any, y?: any, ease?: any) {
        this.moveTo(duration, x, y, ease, true)
        return this;
    },

    moveToDestroyPromise(duration?: any, x?: any, y?: any, ease?: any) {
        this.moveToDestroy(duration, x, y, ease, true);
        return WaitComplete(this._easeMove);
    },

    isRunningMoveTo() {
        return this._easeMove && (this._easeMove.completeEventName === 'moveto.complete');
    },

    isRunningEaseMove() {
        return this.isRunningMoveFrom() || this.isRunningMoveTo();
    },

    moveStop(toEnd?: any) {
        if (!this._easeMove) {
            return this;
        }

        this._easeMove.stop(toEnd);
        return this;
    }
}