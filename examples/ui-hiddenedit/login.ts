...
var CreateLoginDialog = function (scene, config) {
    ...
    userNameField._edit = scene.rexUI.add.hiddenEdit(userNameField);
    ...
    passwordField._edit = scene.rexUI.add.hiddenEdit(passwordField, {
        onOpen(textObject, hiddenEdit) {
            // Can't use passwordField.text because it is masked by dot
            hiddenEdit.setText(password);
        },

        onUpdate(text, textObject, hiddenEdit) {
    ...
    var loginButton = scene.rexUI.add.label({
        ...
        .on('pointerdown', function () {
            userNameField._edit.close();  // Force to close hidden editor
            passwordField._edit.close();  // Force to close hidden editor
            loginDialog.emit('login', userNameField.text, password);
        });
