...
class Demo extends Phaser.Scene {
    ...
    create() {
        ...
        console.log(content.length, characterCache.inCacheCount)

        // Wrap content
        var charPerLine = 24, chunks = [];
        for (var i = 0, cnt = content.length; i < cnt; i += charPerLine) {
            chunks.push(content.substring(i, i + charPerLine));
        }
        content = chunks.join('\n');

        var txt = this.add.bitmapText(10, 10, 'test', content)
    }
