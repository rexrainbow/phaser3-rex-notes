import Phaser from 'phaser';
import TCRPPlugin from '../../plugins/arcadetcrp-plugin'

class PlayerSprite extends Phaser.GameObjects.Line {
    constructor(scene, x, y, fillColor) {
        super(scene, x, y, 30, 0, 0, 0, fillColor);
        this.setLineWidth(4, 15);
        scene.add.existing(this);

        // Arcade
        scene.physics.add.existing(this);
        this.body
            .setSize(30, 30)
            .setDragX(50);
        this.setResetPosition(x, y);
    }

    setResetPosition(x, y) {
        this.resetX = x;
        this.resetY = y;
        return this;
    }

    reset(x, y) {
        if (x !== undefined) {
            this.setResetPosition(x, y);
        }
        this.body.reset(this.resetX, this.resetY);
        return this;
    }

    moveLeft() {
        this.body.setVelocityX(-160);
        return this;
    }

    moveRight() {
        this.body.setVelocityX(160);
        return this;
    }

    jump() {
        if (this.body.touching.down) {
            this.body.setVelocityY(-330);
        }
        return this;
    }

    stop() {
        this.body
            .setVelocity(0, 0)
            .setAcceleration(0, 0);
        return this;
    }
}

class ObstacleSprite extends Phaser.GameObjects.Rectangle {
    constructor(scene, x, y, width, height, fillColor) {
        super(scene, x, y, width, height, fillColor);
        scene.add.existing(this);

        // Arcade
        scene.physics.add.existing(this, true);
        this.body
            .setSize(width, height)
    }

    pushOut(gameObjects) {
        this.scene.physics.add.collider(this, gameObjects, function (obstacle, player) {
            // console.log(`Player ${player.name} hit obstacle`);
        });
    }
}

class Demo extends Phaser.Scene {
    constructor() {
        super({
            key: 'examples'
        })
    }

    preload() { }

    create() {
        var TCRPplugin = this.plugins.get('rexTCRP');
        var recorder = TCRPplugin.addRecorder(this);
        var player = TCRPplugin.addPlayer(this);
        var stepRunner = TCRPplugin.addStepRunner(this);

        // Loop player
        var playerMaxLoop = 3;
        player.on('complete', function () {
            playerMaxLoop--;
            if (playerMaxLoop > 0) {
                player.start();
            }
        });

        var spriteA = (new PlayerSprite(this, 300, 400, 0x00cccc)).setName('A');
        var spriteB = (new PlayerSprite(this, 300, 400, 0xcccc00)).setName('B').setVisible(false);
        var blocker = new ObstacleSprite(this, 400, 500, 800, 50, 0x333333);
        blocker.pushOut([spriteA, spriteB]);

        var print = this.add.text(0, 0, `\
Press Space to start recording/playing
Left/right/up key to move/jump sprite\
`);

        var spaceKey = this.input.keyboard.addKey('SPACE')
        spaceKey.on('down', function (event) {
            spriteB.setVisible(false);

            if (!recorder.isRecording) {
                // Record start
                // spriteA
                recorder.start(1);
                print.setText('Recording');
                var command = ['reset', spriteA.x, spriteA.y];
                recorder.addCommand(command);
                stepRunner.add(command, spriteA);

                var command = ['setVisible', true,];
                recorder.addCommand(command);
                stepRunner.add(command, spriteA);

                // spriteB
                player.stop();
                spriteB.stop().setVisible(false);
            } else {
                // Record stop, player start
                // spriteA
                if (recorder.isRecording) {
                    var command = ['stop'];
                    recorder.addCommand(command);
                    stepRunner.add(command, spriteA);
                }
                recorder.stop();

                // spriteB
                var commands = recorder.getCommands();
                for (var i = 0, cnt = commands.length; i < cnt; i++) {
                    console.log(commands[i].toString());
                }
                player
                    .load(commands, spriteB)
                    .start();
                print.setText('Playing');
            }
        });

        var leftKey = this.input.keyboard.addKey('LEFT');
        leftKey
            .on('down', function () {
                if (recorder.isRecording) {
                    var command = ['moveLeft',];
                    recorder.addCommand(command);
                    stepRunner.add(command, spriteA);
                }
            })

        var rightKey = this.input.keyboard.addKey('RIGHT');
        rightKey
            .on('down', function () {
                if (recorder.isRecording) {
                    var command = ['moveRight',];
                    recorder.addCommand(command);
                    stepRunner.add(command, spriteA);
                }
            })

        var upKey = this.input.keyboard.addKey('UP');
        upKey
            .on('down', function () {
                if (recorder.isRecording) {
                    var command = ['jump',];
                    recorder.addCommand(command);
                    stepRunner.add(command, spriteA);
                }
            })

        // Check position result
        // Store spriteA's position
        var spriteAXs = [],
            spriteAYs = [];
        recorder
            .on('start', function () {
                spriteAXs.length = 0;
                spriteAYs.length = 0;
            })
        this.physics.world.on('worldstep', function () {
            if (recorder.isRecording) {
                spriteAXs.push(spriteA.x);
                spriteAYs.push(spriteA.y);
            }
        });

        // Check spriteB's position
        var spriteBXIndex;
        player.on('start', function () {
            spriteBXIndex = 0;
        })
        this.physics.world.on('worldstep', function () {
            if (player.isPlaying) {
                if (spriteB.x !== spriteAXs[spriteBXIndex]) {
                    console.log(`[${spriteBXIndex}].x A: ${spriteAXs[spriteBXIndex]}, B: ${spriteB.x}`)
                } else {
                    // console.log(`[${spriteBXIndex}] match`)
                }

                if (spriteB.y !== spriteAYs[spriteBXIndex]) {
                    console.log(`[${spriteBXIndex}].y A: ${spriteAYs[spriteBXIndex]}, B: ${spriteB.y}`)
                } else {
                    // console.log(`[${spriteBXIndex}] match`)
                }
                spriteBXIndex++;
            }
        });
        // Check position result
    }

    update() { }
}

var config = {
    type: Phaser.AUTO,
    parent: 'phaser-example',
    width: 800,
    height: 600,
    scale: {
        mode: Phaser.Scale.FIT,
        autoCenter: Phaser.Scale.CENTER_BOTH,
    },
    physics: {
        default: 'arcade',
        arcade: {
            gravity: {
                x: 0, y: 300
            },
            debug: true
        }
    },
    scene: Demo,
    plugins: {
        global: [
            {
                key: 'rexTCRP',
                plugin: TCRPPlugin,
                start: true
            }
        ]
    }
};

var game = new Phaser.Game(config);