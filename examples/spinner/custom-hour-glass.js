...
class Demo extends Phaser.Scene {
    ...
    create() {
        AddHourglassSpinner(this, 400, 300, 100, 100, {
            color: 0xffd740,
        });
...
/**
 * AddHourglassSpinner — Top: inverse mound from top-edge; Bottom: mound pile; same color
 ...
 * @param {number} [options.fillPortion=0.65]
 * @param {number} [options.base=1.7]
 * @param {number} [options.h=1.15]
 * @param {number} [options.slices=18]
 * @param {number} [options.power=2.0]
 * @param {number} [options.gain=1.0]
 * @param {number} [options.flipDeg=180]
 * @param {boolean}[options.showOutline=true]
 * @param {number} [options.outlineColor=0xffffff]
 * @param {number} [options.strokeWidth=2]
 * @param {number} [options.color=0xffffff]  // 上下同色
...
function AddHourglassSpinner(scene, x, y, width, height, options = {}) {
  ...
  return scene.rexSpinner.add.custom({
    ...
    update: function () {
      ...
      const B = R * any.getData('base');
      const H = R * any.getData('h');
