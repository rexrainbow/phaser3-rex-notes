import { GameObjects, Class, Math as Math$1, Geom, Utils as Utils$1, TintModes, Renderer, Game, Scene, Structs, Animations, Textures, Plugins } from 'phaser';
...
const ShaderSourceFS = Renderer.WebGL.Shaders.MultiFrag;
const ShaderSourceVS = Renderer.WebGL.Shaders.MultiVert;
const ShaderAdditionMakers = Renderer.WebGL.ShaderAdditionMakers;
...
const Utils = Renderer.WebGL.Utils;
const BatchHandlerQuad = Renderer.WebGL.RenderNodes.BatchHandlerQuad;
...
class ShatterImage extends Sprite {
    constructor(scene, x, y, key, frame, config) {
        ...
        this.setTriangleCount(GetValue$1(config, 'triangleCount', 8));
...
function DelaunayImageFactory (x, y, texture, frame, config) {
...
function DelaunayImageCreator (config, addToScene) {
...
function DelaunayRenderTextureFactory (x, y, width, height, config) {
...
function DelaunayRenderTextureCreator (config, addToScene) {
    if (config === undefined) { config = {}; }
    if (addToScene !== undefined) {
        config.add = addToScene;
    }
...
class DelaunayImagePlugin extends Plugins.BasePlugin {
...
SetValue(window, 'RexPlugins.GameObjects.DelaunayImage', ShatterImage);
SetValue(window, 'RexPlugins.GameObjects.DelaunayRenderTexture', RenderTexture);

export { DelaunayImagePlugin as default };
