(function (global, factory) {
  ...
  */
  ...
  var BuildHorizontalTextSizer = function BuildHorizontalTextSizer(parentSizer, config) {
    ...
    if (title) {
      var titleAlign = GetValue$2(config, 'align.title', 'right');
      var padding = {
        bottom: !separator && text ? separatorSpace : 0
      };
      parentSizer.add(title, {
        align: titleAlign
      });
    }
    ...
    if (text) {
      var textAlign = GetValue$2(config, 'align.text', 'right');
      parentSizer.add(text, {
        align: textAlign
      });
    }
  ...
  var BuildVerticalTextSizer = function BuildVerticalTextSizer(parentSizer, config) {
    ...
    var titleAlign = GetValue$1(config, 'align.title', 'left');
    var textAlign = GetValue$1(config, 'align.text', 'right');
    var titleProportion = GetValue$1(config, 'proportion.title', 0);
    var textProportion = GetValue$1(config, 'proportion.text', 0);

    if (title) {
      parentSizer.add(title, {
        align: titleAlign,
        proportion: titleProportion
      });
    }

    if (separator) {
      var separatorProportion = GetValue$1(config, 'proportion.separator', 1);
      var padding = {
        left: title ? separatorSpace : 0,
        right: text ? separatorSpace : 0
      };
      parentSizer.add(separator, {
        proportion: separatorProportion,
        padding: padding
      });
    } else {
      // Title aligns to left, text aligns to right, without separator
      if (title && titleAlign === 'left' && titleProportion === 0 && text && textAlign === 'right' && textProportion === 0) {
  ...
  var TitleLabel = /*#__PURE__*/function (_Sizer) {
    ...
    function TitleLabel(scene, config) {
      ...
      if (text || title || separator) {
        ...
        if (textSizer.orientation === 1) {
          // Title, text in vertical layout
          BuildHorizontalTextSizer(textSizer, config);
        } else {
          // Title, text in horizontal layout
    ...
    _createClass(TitleLabel, [{
      ...
      value: function runLayout(parent, newWidth, newHeight) {
        ...
        _get(_getPrototypeOf(TitleLabel.prototype), "runLayout", this).call(this, parent, newWidth, newHeight); // Pin icon-mask to icon game object
      ...
      key: "resize",
      value: function resize(width, height) {
        _get(_getPrototypeOf(TitleLabel.prototype), "resize", this).call(this, width, height); // Resize icon-mask to icon game object
