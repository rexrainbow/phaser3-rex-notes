    ...
    */
    ...
    _createClass(FSM, [{
      ...
      value: function addState(name, config) {
        var getNextStateCallback = GetValue$x(config, 'next', undefined);

        if (getNextStateCallback) {
          this['next_' + name] = getNextStateCallback;
        }

        var exitCallback = GetValue$x(config, 'exit', undefined);

        if (exitCallback) {
          this['exit_' + name] = exitCallback;
        }

        var enterCallback = GetValue$x(config, 'enter', undefined);
