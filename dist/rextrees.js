  ...
  */
  ...
  var Folder = /*#__PURE__*/function (_Sizer) {
    ...
    function Folder(scene, config) {
      ...
      var spaceConfig = GetValue$P(config, 'space');

      // title
      var defaultAlign = _this.orientation === 1 ? 'left' : 'top';
      var align = GetValue$P(config, 'align.title', defaultAlign);
      var expand = GetValue$P(config, 'expand.title', true);
      _this.add(title, {
        proportion: 0,
        align: align,
        expand: expand,
        padding: {
          left: GetValue$P(spaceConfig, 'titleLeft', 0),
          right: GetValue$P(spaceConfig, 'titleRight', 0),
          top: GetValue$P(spaceConfig, 'titleTop', 0),
          bottom: GetValue$P(spaceConfig, 'titleBottom', 0)
        }
      });
      var toggleByTarget = GetValue$P(config, 'toggleByTarget', undefined);
      var toggleClickConfig = GetValue$P(config, 'toggleClickConfig');
      ...
      var customOrigin = GetValue$P(config, 'customChildOrigin', false);
      if (!customOrigin) {
        var origin = !_this.rtl ? 0 : 1;
        child.setOrigin(origin);
      }
      var align = GetValue$P(config, 'align.child', 'left');
      var expand = GetValue$P(config, 'expand.child', true);
      ...
      _this.add(child, {
        ...
        expand: expand,
        padding: {
          left: GetValue$P(spaceConfig, 'childLeft', 0),
          right: GetValue$P(spaceConfig, 'childRight', 0),
          top: GetValue$P(spaceConfig, 'childTop', 0),
          bottom: GetValue$P(spaceConfig, 'childBottom', 0)
        }
      });
      ...
      _this.setTransitionDuration(GetValue$P(transitionConfig, 'duration', 200));
      _this.setExpandCallback(GetValue$P(transitionConfig, 'expandCallback', undefined));
      _this.setCollapseCallback(GetValue$P(transitionConfig, 'collapseCallback', undefined));
      _this.reLayoutTarget = GetValue$P(config, 'reLayoutTarget', undefined);
      ...
      var expanded = GetValue$P(config, 'expanded', undefined);
      if (expanded !== undefined) {
  ...
  var GetGameObjectFromConfig = function GetGameObjectFromConfig(scene, config, key, createCallbackData, defaultCreateGameObjectCallback) {
    var creatorConfig = GetValue$O(config, key);
    var gameObject;
      ...
      value: function updatePenManager(text, wrapMode, wrapWidth, lineHeight, penManager) {
        ...
        for (var i = 0, len = match.length; i < len; i++) {
          ...
          if (curProp.img) {
            ...
            for (var j = 0, jLen = wrapLines.length; j < jLen; j++) {
              segment = wrapLines[j];
              penManager.addTextPen(segment.text, cursorX, cursorY, segment.width, Clone$1(curProp), segment.newLineMode);
