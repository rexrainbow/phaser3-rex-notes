(function (global, factory) {
  ...
  */
    ...
    */
    ...
    _createClass(FSM, [{
      ...
      value: function addState(name, config) {
        var getNextStateCallback = GetValue$m(config, 'next', undefined);

        if (getNextStateCallback) {
          this['next_' + name] = getNextStateCallback;
        }

        var exitCallback = GetValue$m(config, 'exit', undefined);

        if (exitCallback) {
          this['exit_' + name] = exitCallback;
        }

        var enterCallback = GetValue$m(config, 'enter', undefined);
  ...
  Object.assign(methods$3, PaddingMethods, AddChildMethods$3, RemoveChildMethods$3, GetParentSizerMethods, ScaleMethods, FadeMethods, EaseMoveMethods, ShakeMethods, ClickMethods, TouchingMethods, HideMethods, GetShownChildrenMethods);
