  ...
  */
  ...
  var Add = function Add(gameObject, childKey, align, padding, expand, minWidth, minHeight, offsetX, offsetY) {
    ...
    if (IsPlainObject$2(childKey)) {
      ...
      expand = GetValue$3(config, 'expand', true);

      if (!gameObject.isRexSizer) {
        // Get minWidth,minHeight from config
        minWidth = GetValue$3(config, 'minWidth', gameObject._minWidth);
        minHeight = GetValue$3(config, 'minHeight', gameObject._minHeighted);
      }
    }
    ...
    if (IsPlainObject$2(expand)) {
      config.expandWidth = GetValue$3(expand, 'width', false);
      config.expandHeight = GetValue$3(expand, 'height', false);
    } else {
